...
describe('AnagramFinder', function () {
  it('should be able to detect an anagram', function () {
    ...
    // assert.deepStrictEqual(anagramFinder.findAnagrams(['cat', 'dog']), [['c', 'a', 't'], ['d', 'o', 'g']]);
    // assert.deepStrictEqual(anagramFinder.findAnagrams(['cat', 'dog', 'tac', 'abd']), ['cat', 'tac']);
  ...
  it('should convert string to array', function() {
    const anagramFinder = new AnagramFinder('word');
    assert.deepStrictEqual(anagramFinder.stringToArray("string"), ["s", "t", "r", "i", "n", "g"])
  })
});
